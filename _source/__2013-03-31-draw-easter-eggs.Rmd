...
```
This article shows you how to draw an egg with R. It's probably the simplest way to create an animated figure using the [**animation** package](http://yihui.name/animation). The only function used in this package is `ani.pause()`.
...
y = -H * cos(t)
par(mar=rep(0,4))
plot(x, y, type = "l", xlim = c(-1, 1), ylim = c(-1, 1), asp = 1, col = "orange1", lwd = 5, axes = FALSE)
```
...
```{r rotating-egg, fig.show='animate', interval=.1}
library(animation)
egg_rotation = function(H = 1, angle = seq(0,pi*2,length=30), pos = c(0, 0)) {
  t = seq(-pi, pi, by = 0.01)
  for (i in 1:length(angle)) {
    ...
    plot(x1, y1, type = "l", xlim = c(-1, 1), ylim = c(-1, 1), asp = 1, col = cols[flag],
         lwd = 8, axes = FALSE)
    ani.pause(0.1)
  }
}
...
```{r easter-egg, fig.show='animate', interval=.1}
library(animation)
...
```{r draw-3d-egg, rgl=TRUE, warning=FALSE}
