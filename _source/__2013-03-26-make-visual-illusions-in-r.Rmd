...
```{r scintillating-grid-illusion, fig.height=5, fig.width=5}
library(animation)
vi.grid.illusion()
```
...
```{r hermann-grid-illusion, fig.height=5, fig.width=5}
...
```{r lilac-chaser, fig.show='animate', interval=.05, echo=-2}
ani.options(nmax = 20)
ani.options(nmax = 1) # when embedded in HTML, we only need nmax=1
par(mar = c(1, 1, 1, 1))
vi.lilac.chaser()
```
